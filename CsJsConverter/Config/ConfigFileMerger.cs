...
namespace CsJsConversion.Config
{
    ...
    /// <see cref="http://www.codeproject.com/Articles/7406/XmlConfigMerge-Merge-config-file-settings"/>
    /// </summary>
    [Serializable]
	public class ConfigFileMerger
    {
        public ConfigFileMerger(string masterConfigPath) : this(masterConfigPath, null)
        {
        }

        public ConfigFileMerger(string masterConfigPath, string mergeFromConfigPath)
                                 ...
                                 bool makeMergeFromConfigPathTheSavePath)
        {
            ...
            try
            {
                using (var rd = new FileStream(ConfigPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                ...
                && File.Exists(mergeFromConfigPath))
            {
                ...
                var reInstallConfig = new XmlDocument();
                try
                {
        ...
        private readonly string _fromLastInstallConfigPath;
        ...
        public string[] ReplaceXPathValues(string xPath, string replaceWith, Regex regexPattern)
        {
            ...
            if (nodes.Count == 0)
            {
                ...
                var regex = new Regex(@"/configuration/appSettings/add\[\@key=['""](.*)['""]\]/\@value");
            ...
            foreach (XmlNode node in nodes)
            {
                ...
                if (replText != null)
                {
                    ...
                    if (attribute != null)
                    {
                        XmlAttribute keyAttrib = attribute.OwnerElement.Attributes["key"];
                        if (keyAttrib == null)
                        {
                            ret.Add(attribute.OwnerElement.Name + "/@" + node.Name + " set to '" + replText +
                                    "'");
                        }
                        else
                        {
                            ret.Add(attribute.OwnerElement.Name + "[@key='" + keyAttrib.InnerText + "']/@" +
        ...
        private void ClearSpecifiedFileAttributes(string path, FileAttributes fileAttributes)
        {
            File.SetAttributes(path,
                File.GetAttributes(path) & (FileAttributes) ((FileAttributes) 0x7FFFFFFF - fileAttributes));
        }
        ...
        private static void UpdateExistingElementsAndAttribsRecurse(XmlNodeList fromNodes, XmlNode toParentNode)
        {
            ...
            foreach (XmlNode node in fromNodes)
            {
                ...
                if (toNode == null)
                {
                    ...
                    {
                        throw new ApplicationException("node.ParentNode.Name != toParentNode.Name: " +
        ...
        {
            return node.ChildNodes.Cast<XmlNode>().FirstOrDefault(subNode => subNode.NodeType == XmlNodeType.Text);
        }
        ...
        private static readonly Regex _typeParsePattern = new Regex(@"([^,]+),");

        private static XmlNode SelectSingleNodeMatchingNamespaceURI(XmlNode node, XmlNode nodeName,
                                                                    XmlAttribute keyAttrib, int iSameElement)
        {
            ...
            foreach (XmlNode subNode in node.ChildNodes)
            {
                ...
                else if (iNodeNameElements == iSameElement)
                {
                    return subNode;
                }
